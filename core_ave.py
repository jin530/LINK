...
class COREave(SequentialRecommender):
    ...
    def predict_for_fvcore(self, item_seq):
        seq_output = self.forward(item_seq)
        test_item_emb = self.item_embedding.weight
        # no dropout for evaluation
        test_item_emb = F.normalize(test_item_emb, dim=-1)
    ...
    def save_embedding(self, path):
        embedding_weight = self.item_embedding.weight.data.cpu().numpy()
        np.save(path, embedding_weight)

    def load_embedding(self, path):
        embedding_weight = np.load(path)
        self.item_embedding.weight.data.copy_(torch.from_numpy(embedding_weight))